...
import Class_GitOps as git_ops
import Class_Notifier as notifier
import Class_RepoData as repo_data
import Class_SelfUpdate as self_update
import Class_State as state
import Class_Version as version
from Class_Gui import AutoSyncGUI, remote_to_github_web_url
...
# Папка самой программы AutoSync целиком — "Сам проект/" (этот файл лежит уровнем ниже, в Class/,
# поэтому берём .parent.parent, а не .parent). Если она физически оказалась ВНУТРИ отслеживаемого
# пути (как раньше — Tools/AutoSync лежал внутри "Сам проект\Godot_Template", которую и стережём)
# — свои же служебные файлы (Save/autosync.log, Save/state.json, __pycache__/*.pyc,
# Save/autosync_crash.log) были бы триггером собственного пуша программы саму на себя: лог пишется
# на КАЖДОЕ сообщение, из-за чего запись в лог сама вызывала новую проверку/пуш, которая снова
# что-то логировала, и т.д. — отсюда и подозрительные версии/теги. Файлы самой программы из
# слежения полностью исключаем, независимо от имени.
_AUTOSYNC_DIR = Path(__file__).resolve().parent.parent
...
def log(repo_name: str, message: str) -> None:
    if app is None:
        # Окно ещё не создано (RepoWatcher.__init__ вызывается в main() ДО AutoSyncGUI —
        # см. _reconcile_repo_data_on_start) — пишем прямо в тот же autosync.log, который
        # окно подхватит как историю при старте (см. gui.py — _load_log_history).
        line = f"[{datetime.now().strftime('%H:%M:%S')}] [{repo_name}] {message}"
        try:
            log_path = Path(__file__).resolve().parent.parent / "Save" / "autosync.log"
...
class RepoWatcher(FileSystemEventHandler):
    ...
    def _ensure_gitignore_protection(self) -> None:
        """.gitignore и git-индекс живут ОТДЕЛЬНО НА КАЖДОЙ ВЕТКЕ — правка, сделанная на одной
        ветке, не действует на другой. Раньше это вызывалось только один раз при старте
        программы (см. _reconcile_repo_data_on_start), что было достаточно, пока ветка на диске
        не менялась сама. С 22.09, когда AutoSync научился САМ переключать ветку при
        несовпадении (см. _action_push), выяснилось (в тот же день, на Life_Operator): после
        переключения с 'main' на 'alpha/Mzs7' в коммит проскочила .autosync_data — потому что
        .gitignore ветки 'alpha/Mzs7' ещё не содержал нужного исключения (оно было дописано
        раньше, но на другой, тогда текущей, ветке 'main'). Поэтому теперь эта проверка
        вызывается КАЖДЫЙ РАЗ перед коммитом на актуально проверенной ветке — из
        _reconcile_repo_data_on_start (при старте) и из _action_push (сразу после
        автопереключения ветки, до сбора changed_files)."""
        ...
        # исключения — если какая-то из игнорируемых папок (не только .autosync_data — ЛЮБАЯ из
        # _ignored_folders(), включая то, что пользователь сам дописал в config.json) успела
        # попасть в git раньше (обнаружено 21.09 у Godot_Template — .autosync_data коммитился и
        # пушился на GitHub каждый раз), сам .gitignore тут бессилен. Проверяем КАЖДУЮ из них и
        # один раз убираем из индекса автоматически — дальше .gitignore не даст этому
        # повториться. Файлы на диске не трогаются, только git перестаёт их отслеживать.
        for folder in sorted(_ignored_folders()):
            if folder == ".git":
                continue  # саму служебную папку git трогать не пытаемся — это не обычный путь
            try:
                if git_ops.is_tracked(self.repo_path, folder):
                    git_ops.untrack_path(self.repo_path, folder)
                    log(
                        self.name,
                        f"{folder} был случайно закоммичен в git ранее — убран из индекса "
                        f"(файлы на диске не тронуты, уйдёт из GitHub со следующим пушем)",
                    )
            except git_ops.GitError as e:
                log(self.name, f"Не удалось проверить/убрать {folder} из git-индекса: {e}")
    ...
    def _action_push(self, reason: str, force: bool = False, explicit_prefix: Optional[str] = None) -> None:
        ...
        # Важная проверка (найдена 22.09 на Life_Operator): git commit коммитит на ту ветку,
        # которая СЕЙЧАС ФАКТИЧЕСКИ переключена (HEAD) — не обязательно на self.branch. Если в
        # рабочей копии на диске переключена другая ветка, коммит уйдёт не туда, локальная ветка
        # self.branch останется без изменений, а push/тег всё равно "пройдут успешно" (просто
        # ничего реального не подвинут) — снаружи это выглядит как то самое "версия/файлы не
        # совпали" после каждого пуша, без какой-либо явной ошибки. Раньше проверки на это не
        # было вообще, потом (22.09) появилась остановка с сообщением, а теперь (по просьбе
        # пользователя) AutoSync сам переключает ветку на нужную, вместо того чтобы просто
        # сообщать и ждать ручного git checkout.
        try:
            checked_out = git_ops.current_branch(self.repo_path)
        except git_ops.GitError as e:
            checked_out = None
            log(self.name, f"Не удалось определить текущую ветку рабочей копии: {e}")
        if checked_out is not None and checked_out != self.branch:
            log(
                self.name,
                f"В рабочей копии на диске переключена ветка {checked_out!r}, а не {self.branch!r} "
                f"(та, за которой следит AutoSync) — переключаем автоматически...",
            )
            try:
                git_ops.checkout(self.repo_path, self.branch)
            except git_ops.GitError as e:
                # Не переключаем силой (без -f) — если мешают незакоммиченные изменения, лучше
                # остановиться с понятной причиной, чем рискнуть чужими несохранёнными правками.
                self.last_action = "не удалось переключить ветку"
                log(
                    self.name,
                    f"ОСТАНОВЛЕНО: не удалось автоматически переключиться на ветку {self.branch!r}: "
                    f"{e}\nВозможно, мешают незакоммиченные изменения на ветке {checked_out!r} — "
                    f"разберитесь с ними вручную и повторите.",
                )
                return
            log(self.name, f"Переключились на ветку {self.branch!r}")
            # .gitignore/индекс проверялись при старте программы на ТОЙ ветке, что тогда была
            # текущей — после переключения сюда нужна та же проверка заново, на актуальной ветке
            # (см. подробности в _ensure_gitignore_protection), иначе .autosync_data может
            # проскочить в коммит именно на той ветке, куда мы только что переключились.
            self._ensure_gitignore_protection()
...
def main(config_path_: Optional[str] = None) -> None:
    ...
    if config_path_ is None:
        # По умолчанию config.json ищем в Save/ — рядом с самим Class_Watcher.py (Class/) лежит
        # соседняя папка Save/, куда переехали config.json/state.json и логи (22.09, вместе с
        # переносом кода программы в Class/) — а не в текущей рабочей папке процесса: раньше
        # относительный путь "config.json" резолвился от того, откуда был запущен процесс, и при
        # запуске программы не из её собственной папки (например ярлыком) она не находила свой
